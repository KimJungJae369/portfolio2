...
export default function Header() {
  ...
  const HomeClick = (e : React.MouseEvent<HTMLAnchorElement>) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      win.setSwiperState?.({ isHidden: false, isScrollOut: false, isLastSlide: false });
      win.setProjectsState?.({ forceShow: false });
      win.setSectionState?.({ hide: true });
      // 폴백: 직접 컨테이너 숨김 해제
      document.querySelector('.swiperSection')?.classList.remove('hidden-by-projects');
  ...
  const AboutClick = (e : React.MouseEvent<HTMLAnchorElement>) => {
    ...
    if (typeof window !== 'undefined') {
      const win = window as any;
      win.setProjectsState?.({ forceShow: false });
      win.setSwiperState?.({ isHidden: true, isScrollOut: false, isLastSlide: false });
      win.setSectionState?.({ hide: false });
      document.querySelector('.swiperSection')?.classList.remove('hidden-by-projects');
      document.body.classList.remove('section-scrolled-out');
    }
    ...
    if (aboutSection) {
      ...
      setTimeout(() => {
        ...
        if (win && win.swiperRef && typeof win.swiperRef.slideToLoop === 'function') {
          win.swiperRef.slideToLoop(0, 0);
        }
      }, 500);
    }
  }
  ...
  const ProjectsClick = (e : React.MouseEvent<HTMLAnchorElement>) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      win.setSectionState?.({ hide: true });
      // 폴백: 직접 컨테이너 숨김
      document.querySelector('.swiperSection')?.classList.add('hidden-by-projects');
  ...
  const FooterClick = (e : React.MouseEvent<HTMLAnchorElement>) => {
    ...
    if (typeof window !== 'undefined') {
      const win = window as any;
      // Ensure consistent state: hide swiper and enter projects/footer mode
    ...
    if (footerElement) {
      // mark that we're programmatically navigating to footer so swiper ignores scroll events
      document.body.classList.add('footer-nav-scroll');
      ...
      // remove the nav sentinel after animation (fallback 1000ms)
      setTimeout(() => {
