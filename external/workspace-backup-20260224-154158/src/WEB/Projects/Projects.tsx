      ...
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          if (typeof window !== 'undefined') {
            ...
            // if we just left the footer, we want to keep the swiper hidden while
            // the user remains in Projects. a flag ensures scrolling within Projects
            // doesn't prematurely re-show it.
            if (win.footerJustLeft) {
              win.preventSwiperShowInProjects = true;
              // make sure swiper stays hidden/out
              win.setSwiperState?.({ isHidden: false, isScrollOut: true, isLastSlide: false });
            }
            ...
            // clear transient flag if present
            if (win.projectsJustLeft) delete win.projectsJustLeft;
          }
        } else {
          ...
          if (typeof window !== 'undefined') {
            ...
            // clear prevent flag when leaving projects
            if (win.preventSwiperShowInProjects) delete win.preventSwiperShowInProjects;
          }
        }
      });
    };
